...
<html>
			...
			function bbcode(input,bbopen,bbclose) {
				...
				if (typeof document.selection != 'undefined') { // Internet explorer
					...
					if (text.length == 0) {
						...
						range.moveStart('character', bbopen.length+text.length+bbclose.length);
					}
					range.select();
					...
					input.value = input.value.substr(0, start) + bbopen + text + bbclose + input.value.substr(end);
	...
	<body>
		<div id="page">
				...
				<textarea id="textarea" name="bbcode"><?php
					if(isset($_POST['bbcode'])) {
						...
						echo '[h1]Index[/h1]',"\n",
							 ...
							 '[img]http://upload.wikimedia.org/wikipedia/commons/thumb/3/35/Tux.svg/200px-Tux.svg.png[/img]',"\n\n",
							 '[h2]Quote[/h2]',"\n",
							 'You can add a quote :',"\n",
							 '[quote]Quote[/quote]',"\n\n",
							 ...
							 '[iframe=560,315]//www.youtube.com/embed/Pwe-pA6TaZk?rel=0[/iframe]',"\n\n",
							 '[h1]Messages[/h1]',"\n",
			...
			if(isset($_POST['bbcode'])) {
				echo '<h1>Result</h1>',
				     '<div id="result">',bbcode($_POST['bbcode'],array('headers_max_level' => 1)),'</div>';
			}
			?>
		</div>
	</body>
